...
import { DmChucvu } from './../models/danh_muc';
...
export class DmChucvuService {
  ...
  private readonly api = getRoute('danhmuc-chucvu/');
  ...
  list(ma_id: number, filter?: { search: string }): Observable<DmChucvu[]> {
    let params: HttpParams = new HttpParams();
    if (filter) {
      const conditions: OvicConditionParam[] = [
        {
          conditionName: "ten_chucvu",
