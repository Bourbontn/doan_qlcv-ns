...
import { DmDantoc } from './../models/danh_muc';
import { Injectable } from '@angular/core';

@Injectable({
  providedIn: 'root'
})
export class DmDantocService {

  private readonly api = getRoute('danhmuc-dantoc/');
  ...
  constructor(    private http: HttpClient,
    ...
    list(ma_id: number, filter?: { search: string }): Observable<DmDantoc[]> {
      let params: HttpParams = new HttpParams();
      if (filter) {
        const conditions: OvicConditionParam[] = [
          {
            conditionName: "ten_dantoc",
