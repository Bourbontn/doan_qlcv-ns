...
import { DmChucdanh } from './../models/danh_muc';
import { Injectable } from '@angular/core';

@Injectable({
  providedIn: 'root'
})
export class DmChucdanhService {
  private readonly api = getRoute('danhmuc-chucdanh/');
  ...
  constructor(    private http: HttpClient,
    ...
    list(ma_id: number, filter?: { search: string }): Observable<DmChucdanh[]> {
      let params: HttpParams = new HttpParams();
      if (filter) {
        const conditions: OvicConditionParam[] = [
          {
            conditionName: "ten_chucdanh",
